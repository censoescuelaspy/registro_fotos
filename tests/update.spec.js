...
test('muestra la version y permite buscar actualizaciones antes y despues del acceso', async ({ page }) => {
  ...
  await expect(page.getByText('Version 1.7.3', { exact: true })).toBeVisible();
  ...
  await expect(page.getByText('Ya esta usando la version 1.7.3.')).toBeVisible();
  ...
  await expect(page.locator('.topbar .version-badge')).toHaveText('v1.7.3');
...
test('el service worker invalida la cache anterior, recarga y marca la version aplicada', async () => {
  ...
  expect(source).toContain("const CACHE_VERSION = 'cialpa-fotos-v1.7.3'");
...
test('aplica una version nueva y avisa al usuario despues de la recarga', async ({ page }) => {
  ...
  await page.evaluate(async () => {
    ...
    await caches.delete('cialpa-fotos-v1.7.3-shell');
    await caches.open('cialpa-fotos-v1.7.2-shell');
    await navigator.serviceWorker.register('./sw.js', { updateViaCache: 'none' });
  });
  await expect(page.getByText('Aplicacion actualizada correctamente a la version 1.7.3.')).toBeVisible({ timeout: 15000 });
...
test('muestra el aviso cuando la recarga informa la version aplicada', async ({ page }) => {
  await page.goto('/?demo=1&app_updated=1.7.3');
  await expect(page.getByText('Aplicacion actualizada correctamente a la version 1.7.3.')).toBeVisible();
  await expect(page).not.toHaveURL(/app_updated=/);
});
